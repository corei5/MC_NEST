...
def LLM_chat_completion(messages: list[ChatCompletionMessageParam], model: str, temperature: float, **kwargs) -> str:
    # Clear the CUDA cache before running each LLM completion
    torch.cuda.empty_cache()  # Clear GPU memory before running a new task
    
    response = pipeline(messages)
    
    # Handle potential memory optimizations in the response logic as well
...
if __name__ == "__main__":
    ...
    for index, row in df.iloc[96:].iterrows():  # Slice the DataFrame to start at index 15
        ...
        df.to_csv("/nfs/home/rabbyg/CAG/AIME_dataset_exp/output/NE_output/Mistral_22B/GREEDY_rollout_mctsr_NE_Mistral_22B_AIME_from_96.csv", encoding='utf-8', index=False)
